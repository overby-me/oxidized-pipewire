# pw-config --prefix=TEST paths (inline value).
...
"$REF" --prefix=TEST paths > "$TMPDIR/expected" 2>/dev/null
"$RUST" --prefix=TEST paths > "$TMPDIR/actual" 2>/dev/null
sed -i "s|$TMPDIR|TMPDIR|g" "$TMPDIR/expected" "$TMPDIR/actual"
compare "pw-config/paths-prefix-inline"
