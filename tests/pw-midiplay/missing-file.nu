...
try { ^$env.REF ($env.TMPDIR | path join nonexistent.mid) o+e> ($env.TMPDIR | path join expected) }
let e_ref = $env.LAST_EXIT_CODE
try { ^$env.RUST ($env.TMPDIR | path join nonexistent.mid) o+e> ($env.TMPDIR | path join actual) }
