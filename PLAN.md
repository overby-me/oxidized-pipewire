...
**450/450 Nix-level tests passing**, plus 43 internal Rust unit tests:
...
| **Now** | **450/450** | Every test passes; every user-facing tool has byte-identical option parity. Includes: every short-alias of pw-cli's commands, rich-daemon variants of every ls/info subtest, pattern filtering for pw-link, mode-error messages (connect/disconnect), short cluster handling (`-hV` / `-Vh` / `-hp`), `--help=foo` / `--version=foo` / `--remote=foo` getopt errors for every tool, missing-arg errors (`option requires an argument -- 'X'` / `option '--X' requires an argument`), BSD-style getopt parity for pw-v4l2, `--` option-terminator handling, pw-top/pw-dot connect-failure parity (`Can't connect: Host is down` / `can't connect`), pw-cli alias-aware error messages, more SMF fixture coverage, and JSON-dump array/object/special-key edge cases |
